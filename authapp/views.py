...
def enroll(request):
    if not request.user.is_authenticated:
        messages.warning(request,"please Login and try")
        return redirect('/login')
    ...
    if request.method=="POST":
        ...
        reference=request.POST.get('refrence')
        address=request.POST.get('address')
        query=Enrollment(FullName=FullName,Email=email,PhoneNumber=PhoneNumber,DOB=DOB,SelectMembershipplan=member,SelectTrainer=trainer,Reference=reference,Address=address)
        query.save()
